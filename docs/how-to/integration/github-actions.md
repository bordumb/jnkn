      ...
      - name: Scan and analyze
        ...
        run: |
          ...
          if [ -n "$CHANGED" ]; then
            ...
            for f in $CHANGED; do
              echo "### \`$f\`" >> $REPORT
              jnkn blast-radius "file://$f" --format markdown >> $REPORT 2>/dev/null || echo "No dependencies" >> $REPORT
            done
...
- name: Check impact threshold
  run: |
    ...
    for f in $(git diff --name-only origin/main...HEAD | grep -E '\.(py|tf)$'); do
      IMPACT=$(jnkn blast-radius "file://$f" --format json 2>/dev/null | jq '.total_impacted_count // 0')
