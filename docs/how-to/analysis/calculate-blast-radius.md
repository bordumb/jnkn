...
```bash
jnkn blast-radius env:X --max-depth 1
```

Show up to 3 levels:

```bash
jnkn blast-radius env:X --max-depth 3
```
...
```bash
jnkn blast-radius env:DATABASE_URL env:REDIS_URL infra:aws_rds.main
```
...
```bash
jnkn blast-radius env:X --type infra
```

Only show code files:

```bash
jnkn blast-radius env:X --type code
```
...
=== "JSON (default)"
    ...
    ```bash
    jnkn blast-radius env:X --format json
    ```

=== "Markdown"

    ```bash
    jnkn blast-radius env:X --format markdown
    ```

=== "Plain"

    ```bash
    jnkn blast-radius env:X --format plain
    ```

=== "SARIF"

    ```bash
    jnkn blast-radius env:X --format sarif > results.sarif
    ```
...
```bash
IMPACT=$(jnkn blast-radius env:X --format json | jq '.total_impacted_count')
...
```bash
jnkn blast-radius env:X --format json | jq -r '.impacted_artifacts[]' | grep '^file://'
```
...
```bash
# Send to Slack
jnkn blast-radius env:X --format markdown | slack-cli post "#alerts"

# Create Jira ticket
jnkn blast-radius env:X --format json | jira-create-ticket.sh
```
