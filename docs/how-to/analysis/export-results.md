...
```bash
jnkn blast-radius env:X --format json > results.json
```
...
```bash
jnkn blast-radius env:X --format sarif > jnkn.sarif
```
...
```bash
# Pretty print
jnkn blast-radius env:X | jq .

# Count impacted files
jnkn blast-radius env:X | jq '.impacted_artifacts | length'

# Get unique impacted types
jnkn blast-radius env:X | jq -r '.breakdown | keys[]'
```
