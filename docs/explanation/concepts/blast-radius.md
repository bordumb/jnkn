...
```bash
jnkn blast-radius env:DATABASE_URL env:REDIS_URL
```
...
```bash
jnkn blast-radius file://terraform/rds.tf
```
...
```bash
jnkn blast-radius infra:aws_rds.main --reverse
```
...
```bash
jnkn blast-radius env:LEGACY_API_KEY
```
