        ...
        run: |
          ...
          for file in $CHANGED; do
            if [[ "$file" =~ \.(py|tf|yaml|yml)$ ]]; then
              IMPACT=$(jnkn blast-radius "file://$file" --db .jnkn/pr.db --format json 2>/dev/null | jq -r '.total_impacted_count // 0')
              TOTAL_IMPACT=$((TOTAL_IMPACT + IMPACT))
              
              if [ "$IMPACT" -gt 0 ]; then
                REPORT="$REPORT\n### \`$file\` — $IMPACT artifacts\n"
                REPORT="$REPORT$(jnkn blast-radius "file://$file" --db .jnkn/pr.db --format markdown 2>/dev/null || echo '')\n"
              fi
            fi
          done
