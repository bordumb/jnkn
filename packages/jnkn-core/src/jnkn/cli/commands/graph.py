...
@click.option(
    "-o", "--output", default="lineage.html", help="Output file (.html or .dot) for file export"
)
@click.option(
    "--json", "json_mode", is_flag=True, help="Output graph data as JSON to stdout (for VS Code)"
)
def graph(graph_file: str, output: str, json_mode: bool):
    ...
    if json_mode:
            ...
            if hasattr(g, "nodes") and callable(g.nodes):
                ...
                try:
                    ...
                    for n, d in g.nodes(data=True):
                        data["nodes"].append(
                            {"id": n, "name": d.get("name", n), "type": d.get("type", "unknown")}
                        )
                    for u, v, d in g.edges(data=True):
                        data["edges"].append(
                            {"source_id": u, "target_id": v, "confidence": d.get("confidence", 1.0)}
                        )
                except TypeError:
                    ...
                    for i, node_obj in enumerate(nodes):
                        ...
                        data["nodes"].append({"id": nid, "name": name, "type": ntype})
                    ...
                    if hasattr(g, "edge_index_map"):
                        for _, (src_idx, tgt_idx, edge_data) in g.edge_index_map().items():
                            ...
                            conf = (
                                getattr(edge_data, "confidence", 1.0)
                                if hasattr(edge_data, "confidence")
                                else 1.0
                            )
                ...
                json.dumps(
                    {
                        "meta": {"status": "error"},
                        "error": {
                            "message": f"Failed to serialize graph: {str(e)}",
                            "type": str(type(g)),
                        },
                    }
                )
        ...
        click.echo(json.dumps({"meta": {"status": "success"}, "data": data}))
        return
